  ...
  methods: {
    async fetchData() {
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:8000/api/class/${id}`);
        const data = response.data;
        // Set nilai name dari data yang diterima
        this.formData.name = data.name;
        // Set nilai jurusan dari data yang diterima jika diperlukan
        this.formData.jurusan = data.jurusan;
      } catch (error) {
    ...
    submitForm() {
      const id = this.$route.params.id;
      axios.put(`http://127.0.0.1:8000/api/class/${id}`, this.formData)
